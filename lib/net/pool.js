...
Pool.prototype._init = function _init() {
  ...
  this.chain.on('full', () => {
    ...
    console.log('Chain is fully synced (height=%d).', this.chain.height);
  });
...
Pool.prototype.discoverSeeds = async function discoverSeeds(checkPeers) {
  ...
  console.log('discoverSeeds max:', max);
...
Pool.prototype.addLoader = function addLoader() {
  ...
  console.log('Adding loader peer (%s).', peer.hostname());
...
Pool.prototype.createOutbound = function createOutbound(addr) {
  ...
  // console.log('Connecting to %s.', peer.hostname())
...
Pool.prototype.bindPeer = function bindPeer(peer) {
  ...
  peer.on('deparse', (packet) => {
    // console.log('in POOL deparse');
    if (packet.cmd === 'ping') {
      // console.log(packet.cmd);
      this.emit('PING_PACKET', packet.buffer, peer);
    }
    if (packet.cmd === 'ping') {
      // console.log(packet.cmd);
      this.emit('PING_PACKET', packet.buffer, peer);
...
Pool.prototype.handlePacket = async function handlePacket(peer, packet) {
  switch (packet.type) {
    ...
    case packetTypes.PING:
      this.emit('PING_PACKET', packet, peer); //emit before any verification
    ...
    case packetTypes.CMPCTBLOCK:
      ...
      console.log('>>> CMPCTBLOCK packet');
      this.emit('CMPCTBLOCK_PACKET', packet, peer); //emit before any verification
...
Pool.prototype.handleBlockInv = async function handleBlockInv(peer, hashes) {
  ...
  //   hashes.length,
  //   peer.hostname());
  this.emit('block_inv', peer.hostname(), hashes)
  ...
  for (const item of items) {
    ...
    switch (item.type) {
      ...
      case invTypes.WITNESS_BLOCK: {
        ...
        if (!result) {
          console.log('> handleGetData > try sendBlock (BLOCK/WITNESS_BLOCK) > result FALSE !');
          notFound.push(item);
          continue;
        }
        blocks++;
      ...
      case invTypes.CMPCT_BLOCK: {
        ...
        if (height < this.chain.tip.height - 10) {
          ...
          if (!result) {
            console.log('> handleGetData > try sendBlock (CMPCT_BLOCK) > result FALSE !');
            notFound.push(item);
            continue;
          }
          blocks++;
        ...
        if (!block) {
          console.log('> handleGetData > try getItem (CMPCT_BLOCK) > no block !');
          notFound.push(item);
          continue;
        }
...
Pool.prototype.handleCmpctBlock = async function handleCmpctBlock(peer, packet) {
  ...
  let result;
  try {
    console.time('block_init');
    result = block.init();
    console.timeEnd('block_init');
  } catch (e) {
    this.logger.debug(
      'Peer sent an invalid compact block (%s).',
      peer.hostname());
  ...
  this.emit('valid_compact_block', peer, block)

  console.time('block_fillMempool');
  const full = block.fillMempool(witness, this.mempool);
  console.timeEnd('block_fillMempool');
  ...
  if (full) {
    ...
    console.time('pool_this_addBlock');
    await this.addBlock(peer, block.toBlock(), flags);
    console.timeEnd('pool_this_addBlock');
    return;
  }
